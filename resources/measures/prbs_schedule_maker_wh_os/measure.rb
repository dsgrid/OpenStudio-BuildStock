...
# start the measure
class PrbsScheduleMakerWhOs < OpenStudio::Measure::ModelMeasure
  # human readable name
  def name
    # Measure name should be the title case of the class name.
    return 'PrbsScheduleMakerWhOs'
  end

  # human readable description
  def description
    return 'Modified prbs schedule maker that is applicable to water heater dr scenarios'
  end
  ...
  def arguments(model)
    ...
    maximum_hold_duration.setDescription("Specifies the maximum hold duration of the DR state.  Cannot be longer than 6 hrs (360 min); otherwise, input will be overridden.")
  ...
  def run(model, runner, user_arguments)
    ...
    runner.registerValue('nyquist_frequency', 'Nyquist frequency, in min^{-1}', nyquist_frequency)
		...
		default_maximum_hold_duration = 6.0 # hours
...
PrbsScheduleMakerWhOs.new.registerWithApplication
