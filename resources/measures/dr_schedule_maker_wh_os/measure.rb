...
# start the measure
class DrScheduleMakerWhOs < OpenStudio::Measure::ModelMeasure
  # human readable name
  def name
    # Measure name should be the title case of the class name.
    return 'dr_schedule_maker_wh_os'
  end

  # human readable description
  def description
    return 'Modified version of the dr schedule maker os measure that is applicable to water heaters'
  end
  ...
  def arguments(model)
    ...
    schedule_name.setDefaultValue("dr_event_wh")
    args << schedule_name
...
DrScheduleMakerWhOs.new.registerWithApplication
